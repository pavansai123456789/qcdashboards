...
def render_dashboard(device_name):
    ...
    # Get the last actual device ID used for this device name from the database
    # This ensures we check the status of the job associated with the user's input ID, 
    # not just the mock ID from the local state.
    device_id_for_status_check = get_last_device_id_for_name(device_name)

    # If no records exist in the DB for this device name, we fall back to the mock ID
    # to ensure the registration form pre-fills correctly.
    if device_id_for_status_check is None:
        device_info = get_device_info(device_name)
        device_id_for_status_check = device_info['deviceId']
        
    # --- Check Last Job Status ---
    # This determines if the Register Details button should be disabled.
    last_job_status = check_last_job_completion_status(device_id_for_status_check)
    ...
    with col_buttons:
        ...
        with col_comp:
            ...
            if st.button(
                ...
                # Call the new function to update the status in the DB
                if mark_job_completed(device_id_for_status_check):
                    # Clear modal/edit state and rerun to reflect DB change
                    st.session_state.show_register_modal = False
                    st.session_state.editing_weld_id = None
                    st.session_state.confirm_delete_id = None
                st.rerun() 
    ...
    weld_df = fetch_weld_details(device_name) # Use device_name for filtering
